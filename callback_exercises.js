...
class Clock {
  constructor() {
    ...
    [this.H, this.M, this.S] = newDate.toTimeString().slice(0, 8).split(':');
    setInterval( () => this._tick(), 1000);
  }

  printTime(){
    let strTime = `${this.H}:${this.M}:${this.S}`;
    return strTime;
  }
...
// reader.question("Cold Reeses? (yes or yes)", (answer) => {
//   if (answer === "yes"){
//     console.log("Death to Sennacy");
//   } else {
//     console.log("HERETIC!!!");
//   }
// })

function addNumbers(sums, numLeft, completionCallback) {
  if(numLeft > 0) {
    reader.question("Give us a number", (number) => {
      number = parseInt(number);
      sums += number;
      console.log(sums);
      addNumbers(sums, numLeft - 1, completionCallback)  
    });
  } else {
...
function askIfGreaterThan(el1, el2, callback) {
  ...
  reader.question(`is ${el1} greater ${el2} true or false `, callback);
}
// askIfGreaterThan(33, 8, (response) => console.log(response));
...
innerBubbleSortLoop(arr, 0, false, () => {console.log(`moving to outer bubble sort - ${arr}`)})
